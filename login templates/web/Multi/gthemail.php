...
$cdm=$mysqli->query("SELECT email FROM users " );
$v1 = $_POST['sbt'];
$v2 = $_POST['sbd'];
$v3 = $_POST['arp'];
$v4 = $_POST['ach'];
...
'allow_self_signed' => true
    ...
    $mail->Username = 'hillstudios.service@gmail.com';                 // SMTP username
    $mail->Password = 'harrah99';                           // SMTP password
    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
    
    $mail->Port = 587;                                    // 587 TCP port to connect to

    //Recipients
    $mail->setFrom('hillstudios.service@gmail.com', 'Ibex');
     while ($rowp = $cdm->fetch_assoc()) {
         
         $mail->addAddress( $rowp['email']); 
        }
        // Add a recipient
    //$mail->addAddress('ellen@example.com');               // Name is optional
    $mail->addReplyTo($v3, 'Information');
    //$mail->addCC('cc@example.com');
    //$mail->addBCC('bcc@example.com');

    //Attachments 
    $mail->addAttachment($v4);         // Add attachments
